...
async def auth_google(body: GoogleAuthIn, request: Request):
    ...
    return await _google_login_or_create(claims)


async def _google_login_or_create(claims: dict) -> dict:
    """Shared Google account login-or-create (native ID-token + browser OAuth).
...
# ───────────── Google OAuth (browser fallback for Expo Go / web) ─────────────
# Backend-mediated OAuth authorization-code flow for environments where the
# native Google Sign-In SDK is unavailable (Expo Go). The app opens /start in
# an in-app browser → Google consent → Google redirects to /callback → we
# exchange the code, verify the ID token (same helper as the native flow),
# then redirect back into the app via deep link: {return_to}#token=<app JWT>.
_GOOGLE_OAUTH_CLIENT_ID = os.environ.get("GOOGLE_WEB_CLIENT_ID", "")
_GOOGLE_OAUTH_CLIENT_SECRET = os.environ.get("GOOGLE_CLIENT_SECRET", "")
_GOOGLE_OAUTH_CALLBACK_URL = os.environ.get("GOOGLE_CALLBACK_URL", "")
_GOOGLE_AUTH_URL = "https://accounts.google.com/o/oauth2/v2/auth"
_GOOGLE_TOKEN_URL = "https://oauth2.googleapis.com/token"
_OAUTH_STATE_TTL_SECONDS = 600


def _allowed_google_return_url(value: str) -> bool:
    """Strict allowlist for the app deep link. Prevents open redirects."""
    try:
        p = urlparse(value)
    except Exception:
        return False
    path = (p.path or "").rstrip("/")
    if p.scheme == "exp":  # Expo Go deep link: exp://host:port/--/oauth/callback
        return path.endswith("--/oauth/callback") or path.endswith("/oauth/callback")
    if p.scheme == "frontend":  # production build scheme (app.json)
        return (p.netloc + p.path).strip("/") == "oauth/callback"
    if p.scheme in ("http", "https"):
        host = p.netloc.split(":")[0]
        allowed_hosts = {"localhost", "127.0.0.1"}
        extra = os.environ.get("GOOGLE_OAUTH_ALLOWED_ORIGINS", "")
        allowed_hosts.update(h.strip() for h in extra.split(",") if h.strip())
        if host in allowed_hosts or host.endswith(".emergentagent.com"):
            return path == "/oauth/callback"
    return False


@api_router.get("/auth/google/oauth/start")
async def google_oauth_start(request: Request, return_to: str):
    """Kick off the Google consent flow. `return_to` is the app deep link we
    redirect to after login (validated against a strict allowlist)."""
    await rate_limit(request, "auth:google_oauth", max_calls=30, window_seconds=3600)
    if not _GOOGLE_OAUTH_CLIENT_ID or not _GOOGLE_OAUTH_CALLBACK_URL:
        raise HTTPException(status_code=503, detail="Google OAuth is not configured on the server")
    if not _allowed_google_return_url(return_to):
        raise HTTPException(status_code=400, detail="Invalid return URL")

    state = secrets.token_urlsafe(32)
    nonce = secrets.token_urlsafe(32)
    # Purge abandoned states older than the TTL.
    await db.oauth_states.delete_many(
        {"created_ts": {"$lt": time.time() - _OAUTH_STATE_TTL_SECONDS}}
    )
    ...
    params = {
        ...
        "redirect_uri": _GOOGLE_OAUTH_CALLBACK_URL,
        "response_type": "code",
...
async def google_oauth_callback(
    ...
    # Single-use state: read then immediately delete.
    pending = await db.oauth_states.find_one({"state": state})
    if pending:
        await db.oauth_states.delete_one({"state": state})
    if not pending or (time.time() - float(pending.get("created_ts", 0))) > _OAUTH_STATE_TTL_SECONDS:
        raise HTTPException(status_code=400, detail="Expired or invalid OAuth state")

    return_to = pending["return_to"]
    if error or not code:
        return RedirectResponse(f"{return_to}#error=google_denied", status_code=302)
    if not _GOOGLE_OAUTH_CLIENT_SECRET:
    ...
    # Verify signature/audience/expiry with the same helper as the native flow.
    if _GOOGLE_OAUTH_CLIENT_ID:
        _GOOGLE_AUDIENCES.add(_GOOGLE_OAUTH_CLIENT_ID)
    ...
    result = await _google_login_or_create(claims)
    # App JWT goes in the URL FRAGMENT — never sent to any server, never logged
    # by proxies. The app reads it from the deep link and clears it.
    return RedirectResponse(f"{return_to}#token={result['token']}", status_code=302)
